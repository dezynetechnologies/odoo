    ...
    def _from(self):
        from_str = """
            ...
            (SELECT
                ...
                SUM(case when total_mm = 0 OR total_offon_mm = 0  then (onsite_allowance + offshore_salary) else (case when geography::text = 'offon' then (offon_mm/total_offon_mm)*onsite_allowance + (offon_mm/total_mm)*offshore_salary else (offshore_mm/total_mm)*offshore_salary end) end) as exp_cost,date,
