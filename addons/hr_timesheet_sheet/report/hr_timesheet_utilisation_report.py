...
class hr_timesheet_utilisation_report(osv.osv):
    ...
    _columns = {
        ...
        'total_offshore_mm' : fields.float('Offshore Man Months'),
        'total_offon_mm' : fields.float('Offon Man Months'),
        'total_mm' : fields.float('Total Man Months'),
        'total_nonbench_mm' : fields.float('Non-Bench Man Months'),
    ...
    def _from(self):
        from_str = """
                (SELECT row_number() OVER() AS id,
                    ...
                    case when t.billing_perc != 0 then ( case when t.geography::text = 'offshore' then ( ((t.date_to - t.date_from + 1)*t.allocation_perc*t.billing_perc)/(( case when date_part('days',date_trunc('month',t.date_to) + '1 month'::interval - date_trunc('month',t.date_to)) < 30 then 28::numeric else 30::numeric end)*100*100) ) else 0::numeric end) else  0::numeric  end as offshore_billed_mm,
		            case when t.billing_perc != 0 then ( case when t.geography::text = 'offon' then ( ((t.date_to - t.date_from + 1)*t.allocation_perc*t.billing_perc)/(( case when date_part('days',date_trunc('month',t.date_to) + '1 month'::interval - date_trunc('month',t.date_to)) < 30 then 28::numeric else 30::numeric end)*100*100) ) else 0::numeric end) else  0::numeric  end as offon_billed_mm,
                    case when t.billing_perc != 0 then (((t.date_to - t.date_from + 1)*t.allocation_perc*t.billing_perc)/(( case when date_part('days',date_trunc('month',t.date_to) + '1 month'::interval - date_trunc('month',t.date_to)) < 30 then 28::numeric else 30::numeric end)*100*100) ) else 0::numeric end as total_billed_mm,
                    case when t.geography::text = 'offshore' then ( (t.date_to - t.date_from + 1)*t.allocation_perc/(( case when date_part('days',date_trunc('month',t.date_to) + '1 month'::interval - date_trunc('month',t.date_to)) < 30 then 28::numeric else 30::numeric end)*100) ) else 0::numeric end as total_offshore_mm,
                    case when t.geography::text = 'offon' then ( (t.date_to - t.date_from + 1)*t.allocation_perc/(( case when date_part('days',date_trunc('month',t.date_to) + '1 month'::interval - date_trunc('month',t.date_to)) < 30 then 28::numeric else 30::numeric end)*100) ) else 0::numeric end as total_offon_mm,
                    ((t.date_to - t.date_from + 1)*t.allocation_perc)/(( case when date_part('days',date_trunc('month',t.date_to) + '1 month'::interval - date_trunc('month',t.date_to)) < 30 then 28::numeric else 30::numeric end)*100) as total_mm,
                    (case when t.sap_project_code::text = 'bench' then 0::numeric else ((t.date_to - t.date_from + 1)*t.allocation_perc)/(( case when date_part('days',date_trunc('month',t.date_to) + '1 month'::interval - date_trunc('month',t.date_to)) < 30 then 28::numeric else 30::numeric end)*100) end) as total_nonbench_mm,
